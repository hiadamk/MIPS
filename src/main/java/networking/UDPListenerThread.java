...
class UDPListenerThread implements Runnable{
				...
				/**
				 * creates a listener thread that continuously recieves udp packets 
				 * and places contents into a queue as strings
				...
				public UDPListenerThread(DatagramSocket socket, Queue<String> feedQueue) {
					...
					if (ipaddr == null) {throw new IllegalArgumentException("Null ip, socket is not connected to anything!");}
					...
					try {
						while (running){
							...
							if (s.startsWith(prefix) && s.endsWith(suffix)) {
								s = s.substring(prefix.length(), s.length()-suffix.length()-1); //rids prefix and suffix
