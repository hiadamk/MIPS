...
public class ProjectileFX {
  ...
  /**
   * Manages rocket images so that they fly up and off the screen then after a duration target the
   * entity it was aimed at
   *
  ...
  private void renderRocket(Rocket r) {
    ...
    if (r.getTime() < r.getMaxTime() * launchDuration) {
      ...
      if (!r.isLaunched()) {
        ...
        renderer.setIsoCoord(
            rendCoord, entityLoc.getX(), entityLoc.getY(), rocketSpriteWidth, rocketSpriteHeight);
      ...
      gc.drawImage(
          upwardRocketImages.get(r.getCurrentFrame() % upwardRocketImages.size()),
          launchX,
          launchY);
    }
    // HIT TIME
      ...
      double currentTime =
          (r.getTime() - (r.getMaxTime() - r.getMaxTime() * hitDuration))
              / (r.getMaxTime() * hitDuration);
      double launchY = r.getEndLocation().getY() * currentTime;
      // System.out.println("DOWN:"+currentTime);
      gc.drawImage(
          downwardRocketImages.get(r.getCurrentFrame() % downwardRocketImages.size()),
          launchX,
          launchY);
    }
  }
