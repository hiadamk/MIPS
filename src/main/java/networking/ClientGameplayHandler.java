...
public class ClientGameplayHandler {
	...
	/** Handles the in-game communication session to the server receiving data to a position queue and a collision queue
	 * and passing sending data from the key press queue.
	 ...
	 */
	public ClientGameplayHandler( Queue<String> positionQueue, Queue<String> collisionQueue, BlockingQueue<Integer> keypressQueue) {
	...
	public void start() {
		...
		if (connect()!=0){return;} //Timeout attempt at connecting.
		started  = true;
	...
	private int connect(){
		...
		try {
			...
			socket.setSoTimeout(5000); //server must connect within 5 seconds
