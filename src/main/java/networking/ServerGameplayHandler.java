...
public class ServerGameplayHandler {
	...
	/** Handles the in-game communication session to the client sending data in position queue and collision queue
	 * and passing received data to the key press queue.
	 * the current model has a listener per client, listening to a distinct port for each client.
	 ...
	 */
	public ServerGameplayHandler(String address, int port, Queue<String> positionQueue, Queue<String> collisionQueue, BlockingQueue<Integer> keypressQueue) throws UnknownHostException{
	...
	/**
	 * Starts the session with the client. Client must also be waiting to started
	 */
	public void start() {
		//don't do anything if threads are already running
		if (started) {return;}
		//connect socket to client
		while(connect()!=0){} //may update this to a timeout-style attempt at connecting.
		started = true;
	...
	public void stop() {
		...
		collisionQueue.add(stopCode); // sends stopcode on outgoingqueue to client. done twice to ensure transmission because UDP
