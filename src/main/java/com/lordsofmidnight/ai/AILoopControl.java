...
public class AILoopControl extends Thread {
   ...
   * the game.
   * @param controlIds The set of com.lordsofmidnight.main Ids that the AI will control.
   ...
   * @throws IllegalArgumentException gameAgent array contains duplicate com.lordsofmidnight.main
   * IDs.
   * @throws IllegalStateException Cannot have more than one mipsman.
   * @throws IllegalStateException The control ID does not match an agent com.lordsofmidnight.main
   * ID.
   * @author Lewis Ackroyd
   */
    ...
    for (Entity e : gameAgents) {
      if (!ids.add(e.getClientId())) {
        throw new IllegalArgumentException(
            "gameAgent array contains duplicate com.lordsofmidnight.main IDs.");
      }
    }
    ...
    for (int i = 0; i < controlIds.length; i++) {
      ...
      if (agentNotFound) {
        throw new IllegalStateException(
            "The control ID does not match an agent com.lordsofmidnight.main ID.");
      }
    }
  }
  ...
  private void setDirection(Direction direction, Entity ent) {
    ...
    if (direction != ent.getDirection() && !ent.isDirectionSet()) {
      if (DEBUG) {
        System.out.println(
            counter++ + " " + direction + " " + ent.getClientId() + " " + ent.getLastGridCoord()
                + " " + ent.getLocation());
      }
  ...
  private Direction confirmOrReplaceDirection(Direction oldDirection, Point currentLocation,
      Direction dir) {
