...
import static server.NetworkUtility.CLIENT_M_PORT;
...
public class ServerBrowser extends Thread {
  ...
  /**
   * Adds servers to a list by repeatedly checking for a message in the group
   * and getting the IP of a server from it.
   */
  ...
  public void run() {
      ...
      try {
        while (!isInterrupted()) {
          System.out.println("Getting a server address");
